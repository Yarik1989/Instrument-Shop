...
public class ProductDaoMySql implements ProductDao {
	...
	// public static final String SQL_FILTER_FROM_PART = "products p, categories
	// c, manufacturers m";
	...
	private Collection<Product> getAllByQuery(String query, List<Object> args, Connection con) throws SQLException {
	...
	@Override
	public Collection<Product> getByFilter(ProductFilterBean filter, Connection con) throws SQLException {
		SqlStatementBuilder builder = buildCompleteStatement(filter);
		return getAllByQuery(builder.buildSql(), builder.getArgs(), con);
	}

	private SqlStatementBuilder buildStatementWithoutSelectPart(ProductFilterBean filter) {
		...
		if (filter.getCategoryIds() != null && !filter.getCategoryIds().isEmpty()) {
			query.addWhere(buildORExpression(filter.getCategoryIds().size(), "c.id = ?"),
					filter.getCategoryIds().toArray());
		}
		if (filter.getManufacturerIds() != null && !filter.getManufacturerIds().isEmpty()) {
			query.addWhere(buildORExpression(filter.getManufacturerIds().size(), "m.id = ?"),
		...
		if (filter.getSortMode() != null) {
			query.addOrder(filter.getSortMode().getSqlFieldName(), filter.getSortMode().isAscending());
		}
	...
	@Override
	public int getCountWithoutLimit(ProductFilterBean filter, Connection con) throws SQLException {
